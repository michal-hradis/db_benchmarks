...
def main():
    ...
    for page in data["elements"][1:]:
        ...
        for paragraph in layout.regions:
            ...
            if not chunks:
                chunks.append(
                    {"text": "", "start_page_id": page["id"], "from_page": page["pageIndex"], "order": len(chunks)})

            paragraph_lines = [line for line in paragraph.lines if
                               line.transcription and line.transcription_confidence > args.line_confidence]
    ...
    if len(chunks) > 0:
        ...
        print(
            f'{data["elements"][0]["id"]}, {chunk_count}, {chunk_sum}, {chunk_avg:.2f}, {chunk_min}, {chunk_median}, {chunk_max}')
    else:
